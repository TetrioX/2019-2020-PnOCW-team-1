...
dfHSV = pd.DataFrame(dict(Hue=h1, Saturation=s1, Value=v1))
figHSV = px.scatter_matrix(dfHSV, dimensions=["Hue","Saturation","Value"])
figHSV.show()
"""
### SAT/HUE
dfHSL = pd.DataFrame(dict(Hue=h2,Lightness=l2, Saturation=s2))
figHSL = px.scatter(dfHSL, x='Hue', y='Saturation',
        range_x = [0,360], range_y = [0,1])
figHSL.show()
### LIGHT/HUE
figHSL2 = px.scatter(dfHSL, x='Hue', y='Lightness',
        range_x = [0,360], range_y = [0,1])
figHSL2.show()
### SAT/LIGHT
