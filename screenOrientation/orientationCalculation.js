...
const getOrientation = function (corners) {
	...
	const tiltDir = getTiltDir(corners);
	let zRotation = null;
...
const getCenter = function (corners) {
	const values = Object.keys(corners).map(function (key) {
			return corners[key]
		}),
...
const getLine3D = function(pos1, pos2) {
	const p = getDirVector(pos1, pos2);
	return { p: p, q: pos1 }
};
...
const normalizeVector = function(vector) {
	const len = calcDistance(vector);
	for (let key in vector) vector[key] = vector[key]/len;
	return vector
};

const calcAngles = function (corners) {
	const AB = normalizeVector(getDirVector(corners.A, corners.B)),
...
const getTiltDir = function(corners) {
	xTilt = corners.A.z !== corners.D.z;
	yTilt = corners.A.z !== corners.B.z;
	zTilt = corners.A.y !== corners.B.y;
