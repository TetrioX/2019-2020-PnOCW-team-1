...
const getCenter = function (corners) {
	const AC = getLine(corners.A, corners.C),
...
const getAngle = function (pos1, pos2, pos3) {
	const u = {x: pos2.x - pos1.x, y: pos2.y - pos1.y, l: getDistance(pos2, pos1)},
...
const transfer2Dto3D = function(corners) {
	...
	const A = get3DCoordinate(corners.A),
	...
	const verh = 675/339.5;
	const v = verh * Math.sqrt(1 + verh**2) * 700**2; // Taak: Schatting maken van r en verhouding meekrijgen
	...
	let opl = nerdamer.solveEquations(eq).toString();
	
	const str = [];
	let substr = '';
	for (let character of opl)
