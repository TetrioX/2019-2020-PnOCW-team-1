...
async function findScreen(imgs, demand_same_size = false) {
	...
	const screenIds = [0, 1, 2]; // searchID(imgs)
	...
	for (let i = 0; i < diff.buffers.length - 1; ++i) {
		...
		dict[screenIds[i]] = scrread.screenReading(diff.buffers[i], diff.buffers[i + 1], tempResult[i], diff.dimensions);
