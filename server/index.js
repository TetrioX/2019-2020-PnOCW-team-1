...
let saveDebugFiles = argv['save-debug-files'];
let gridPause = argv['grid-pause'];
let debugPath = './debug';
let debugDirPromise = new Promise(function (resolve, reject) {
...
let io = socket(server, {pingInterval: 200});
let slaves = {};
let slaveSockets = {};
let number = 0;
...
io.of('/master').use(function (socket, next) {
    const passwd = socket.handshake.query.passwd;
...
let masterIo = io.of('/master').on('connect', function(socket){
    ...
    async function calibrate(numberOfRows, numberOfColumns) {
      ...
      const nbOfColorCombs = Object.keys(slaves).length * 10;
      // calculate how many pictures should be taken
      const nbOfPictures = Math.ceil(Math.log(nbOfColorCombs + possibleColors.length)/Math.log(possibleColors.length));
      const allColorCombinations = getColorComb(nbOfPictures);
      let screens = {};
      let colorCombs = {};
          ...
          if (numberOfRows == null && numberOfColumns == null) {
              ...
              let waitForDim = new Promise(function (resolve, reject) {
                  slaveSockets[slave].emit('getDim', 0, function (callBackData) {
                      const h = callBackData.height;
                      const w = callBackData.width;
                      const wDivH = w / h;
                      if (1.75 <= wDivH) {
                          numberOfRows = 2;
                          numberOfColumns = 4
                      } //Math.round(wDivH * 2)
                      else if (1.25 <= wDivH && wDivH < 1.75) {
                          numberOfRows = 2;
                          numberOfColumns = 3
                      } else if (0.75 <= wDivH && wDivH < 1.25) {
                          numberOfRows = 3;
                          numberOfColumns = 3
                      } else if (0.5 <= wDivH && wDivH < 0.75) {
                          numberOfRows = 3;
                          numberOfColumns = 2
                      } else if (wDivH < 0.5) {
                          numberOfRows = 4;
                          numberOfColumns = 2
                      } //Math.round((1 / wDivH) * 2)
                      else {
      ...
      const pictures = await takePicture(nbOfPictures);
      ...
      let matrices = await imgprcssrgb.doImgDiff(pictures, false, false);
      matrices = matrices.matrix;
      if (saveDebugFiles) {
          fs.writeFile(debugPath+`/matrixes.json`, JSON.stringify(matrices), (err) => {if (err) console.log(err)})
      }
      const squares = scrnread.getScreens(matrices, screens, colorCombs, possibleColors.length);
      console.log(squares);
    ...
    // and pictures a list with all taken pictures
    async function takePicture(n) {
        ...
        while (true) {
            let picPromise = new Promise(function (resolve, reject) {
                ...
                setTimeout(() => reject(new Error("Failed to take picture")), 2000);
            });
            ...
            if (i < n) {
                ...
                // wait until all screens have changed
    ...
    socket.on('changeBackgroundOfAllSlaves', async function (data) {
        const screens = await calibrate(data.numberOfRows, data.numberOfColumns);
    ...
    socket.on('triangulate', async function (data) {
        ...
        const slavedata = screenorientation.getScreens(screens);
        console.log(slavedata);
        const angles = delaunay.getAngles(slavedata);
        console.log(angles);
        Object.keys(slaves).forEach(function (slave, index) {
        ...
        socket.emit('takePictures', {slaves: {0: 'm'}}, function (callbackData) {
            socket.emit('takePictures', {slaves: slaves},
                function (callbackData) {
                    ...
                    let imgs = [`./Pictures/slave-m.png`]; // If this picture doesnot exist an error may be send
                    for (let key in slaves) imgs.push(`./Pictures/slave-${slaves[key]}.png`); // Implement all slave pictures
...
let slaveIo = io.of('/slave').on('connect', function (socket) {
...
function createColorGrid(nbrows, nbcolumns, allColorCombinations, slaveID) {
    ...
    for (let i = 0; i < nbrows; i++) {
        ...
        for (let j = 0; j < nbcolumns; j++) {
            const colorComb = allColorCombinations.pop();
    ...
    const cornBorder = allColorCombinations.pop();
    const sideBorder = allColorCombinations.pop();
