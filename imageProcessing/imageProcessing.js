...
async function doImgDiff(imgs, demand_same_size=false) {
    ...
    const extend = 20 //The number of pixels we want in the largest dimension.
    ...
    for(let i = 0; i < imgs_buffs.length - 1; ++i) {
        ...
        to_file_promises.push( sharp(tempResult[i], output_meta).toFile(`../imageProcessing/Result/diff-${i+1}.png`) )
