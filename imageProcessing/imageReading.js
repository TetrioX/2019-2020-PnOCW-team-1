...
const precision = 30; // reference number to determine difference precision
...
const imageReading = function(buff1, buff2, buff3, channel) {
	...
	for (let i = 0; i < buff1.length; i += channel) {
		const lab1 = [buff1[i], buff1[i + 1], buff1[i + 2]];
		const lab2 = [buff2[i], buff2[i + 1], buff2[i + 2]];

		// factor 5 is hier gekozen om waarden die wit zijn visueel duidelijk wit te maken
